...
def get_requests_session(auth=False):
    ...
    if auth and config.cachito_auth_type == 'kerberos':
        ...
        log.debug(
            'Configuring the requests session\'s authentication with the principal=%r, '
            'keytab=%r, and hostname_override=%r',
            config.cachito_kerberos_principal,
            os.environ['KRB5_CLIENT_KTNAME'],
            config.cachito_kerberos_hostname_override,
        )
        session.auth = requests_kerberos.HTTPKerberosAuth(
            hostname_override=config.cachito_kerberos_hostname_override,
            mutual_authentication=requests_kerberos.OPTIONAL,
            principal=config.cachito_kerberos_principal,
        )
